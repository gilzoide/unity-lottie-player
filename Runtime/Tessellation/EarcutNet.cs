...
namespace Gilzoide.LottiePlayer.Tessellation.EarcutNet
{
    ...
    public unsafe struct EarcutJob : IJob
    {
        ...
        private readonly Node* SplitPolygon(Node* a, Node* b)
        {
            var a2 = CreateNode(a->i, a->position);
            var b2 = CreateNode(b->i, b->position);
            var an = a->next;
            var bp = b->prev;
        ...
        private readonly Node* InsertNode(int i, float2 position, Node* last)
        {
            var p = CreateNode(i, position);
        ...
        private readonly Node* CreateNode(int i, float2 position)
        {
            Node* node = AllocatorManager.Allocate<Node>(NodeAllocator.Allocator.Handle, 1);
            *node = new Node(i, position);
            return node;
        }
